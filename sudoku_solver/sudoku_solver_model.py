...
class Box:
    ...
    def possible_values(self) -> Set[int]:
        ...
        for neighbor in self.neighbors:
            if neighbor.known_value:
                self.possible_values = self._possible_values - {
                    neighbor.known_value
                }
    ...
    def combinations_size_2(self) -> List[Set[int]]:
        ...
        if self._combinations_size_2 is None:
            self._combinations_size_2 = [
                set(combo) for combo in combinations(self.possible_values, 2)
            ]
    ...
    def combinations_size_3(self) -> List[Set[int]]:
        ...
        if self._combinations_size_3 is None:
            self._combinations_size_3 = [
                set(combo) for combo in combinations(self.possible_values, 3)
            ]
    ...
    def combinations_size_4(self) -> List[Set[int]]:
        ...
        if self._combinations_size_4 is None:
            self._combinations_size_4 = [
                set(combo) for combo in combinations(self.possible_values, 4)
            ]
...
class Grid:
    ...
    def generate_boxes() -> List[Box]:
        ...
        return [Box(col, row) for col in range(1, 10) for row in range(1, 10)]

    def get_rows(self) -> List[List[Box]]:
        """Initialize the list of rows of boxes."""
        return [
            [box for box in self.boxes if box.row == i] for i in range(1, 10)
        ]

    def get_cols(self) -> List[List[Box]]:
        """Initialize the list of columns of boxes."""
        return [
            [box for box in self.boxes if box.col == i] for i in range(1, 10)
        ]
    ...
    def assign_neighbors(self):
        ...
        for box in self.boxes:
            ...
            box.neighborhood_sets = [
                box.row_neighbors, box.col_neighbors, box.block_neighbors
            ]
                ...
                for neighborhood in box.neighborhood_sets:
                    for neighbor in neighborhood:
                        if neighbor.possible_values == box.possible_values:
                            for non_matching_neighbor in neighborhood - {
                                    neighbor
                            }:
    ...
    def check_block_col_row_intersections(self):
        ...
        for box in self.grid.boxes:
            ...
            for value in box.possible_values:
                self.check_overlapping_neighbors_for_unique_value(
                    value, box.block_neighbors, box.col_neighbors)
                self.check_overlapping_neighbors_for_unique_value(
                    value, box.block_neighbors, box.row_neighbors)

    @staticmethod
    def check_overlapping_neighbors_for_unique_value(value: int,
                                                     neighborhood_1: Set[Box],
                                                     neighborhood_2: Set[Box]):
        """..."""
        # Get all possible values of boxes in neighborhood 1, excluding boxes from neighborhood 2.
        neighborhood_1_minus_2_possible_values = set().union(
            *[box.possible_values for box in (neighborhood_1 - neighborhood_2)])
        # Get all possible values of boxes in neighborhood 2, excluding boxes from neighborhood 1.
        neighborhood_2_minus_1_possible_values = set().union(
            *[box.possible_values for box in (neighborhood_2 - neighborhood_1)])
        ...
        for box in self.grid.boxes:
            ...
            for neighbor in box.neighbors:
                if box.known_value == neighbor.known_value:
                    raise InvalidGrid(
                        f'Duplicate value: {box} and {neighbor} both are {box.known_value}'
                    )
...
def input_values_into_grid(grid: Grid, known_values: List[Tuple[int, int,
                                                                int]]):
    ...
    for box in known_values:
        for other in (set(known_values) - {box}):
            if box[2] == other[2]:
                ...
                if (box[0] - 1) // 3 == (other[0] - 1) // 3 and (
                        box[1] - 1) // 3 == (other[1] - 1) // 3:
...
def solve(
    known_values: List[Tuple[int, int,
                             int]]) -> List[Tuple[int, int, int]] or str:
